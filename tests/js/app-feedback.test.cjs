...
function boot(extra) {
	...
	const trigger = {
		className: 'snk-nav-footer__trigger',
		getAttribute(k) {
			return this[k] || null;
		},
		setAttribute(k, v) {
	...
	trigger['aria-controls'] = 'snk-feedback-menu';
	const menu = {
		id: 'snk-feedback-menu',
		hidden: true,
		contains() {
			return false;
		},
	...
	nodes.set('snk-feedback-menu', menu);
	doc.querySelector = (sel) => {
		if (sel === '.snk-nav-footer__trigger') {
			return trigger;
		}
		return null;
	};
	doc.getElementById = (id) => {
		if (id === 'snk-feedback-menu') {
			return menu;
		}
		return nodes.get(id) || null;
	};
...
function main() {
	...
	const upgradeAck = sandbox.SbdAppFeedback.sanitizePageUrl(
		'/apps/snackcheck/?IKnowThatThisIsABigInstanceAndTheUpdateRequestCouldRunIntoATimeoutAndHowToRestoreABackup=IAmSuperSureToDoThis'
	);
	assert(upgradeAck === '/apps/snackcheck/', 'strips Nextcloud web-upgrade ack query key');
